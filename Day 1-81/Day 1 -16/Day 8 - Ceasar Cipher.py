...
def ceasar(text, shift, direction):
    ...
    for char in text:
        if char in alphabet:  # allows only letters in alphabet to be changed
            position = alphabet.index(char)     # variable that determines the position of looped
                                                # char from the text in the alphabet
            new_position = (position + shift) % 26  # creates new variable with shifted position. % 26
                                                    # if too big shift number
