from PySide6.QtWidgets import (QMainWindow, QFileDialog, QLabel, QWidget, QGridLayout, QApplication, QGraphicsScene,
                               QGraphicsProxyWidget, QGraphicsTextItem, QStackedWidget)
...
class DraggableLabel(QLabel):
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() == Qt.LeftButton:
            ...
            print(f'Coordinates: {self.body.x(), self.body.y()} - event.pos(): {event.pos()}')
...
class LabelWithRotator(QWidget):
    def __init__(self, parent):
        super().__init__()
        self.setStyleSheet("background:transparent")  # ;background-color: rgba(255, 0, 0, 40)
